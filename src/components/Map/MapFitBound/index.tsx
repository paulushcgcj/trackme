...
interface MapFitBoundProps {
  polygons: FeatureCollection[];
  defaultLocation: LatLngExpression;
  defaultZoom: number;
}

const MapFitBound: React.FC<MapFitBoundProps> = ({ polygons, defaultLocation, defaultZoom }) => {
  const map = useMap();
  ...
  useEffect(() => {
    if (polygons.length > 0) {
      ...
      const bounds = L.featureGroup(polygons.map((geoJson) => L.geoJSON(geoJson))).getBounds();
